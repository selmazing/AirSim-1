...
namespace msr
{
	namespace airlib
	{
		...
		private: // methods
			...
			virtual real_T setAirspeed(const Kinematics::State& kinematics)
			{
				double airspeed = sqrt(pow(kinematics.twist.linear(0), 2) * pow(kinematics.twist.linear(1), 2) * pow(kinematics.twist.linear(2), 2));
				return airspeed;
			}
			...
			virtual void createAeroForces(const LinearAeroDerivatives& derivatives, const Dimensions& dimensions, const Kinematics::State& kinematics, ControlSurface& output)
			{
				output_.aero_force_.lift = dyn_pressure_ * dimensions.main_plane_area * (
					...
					(derivatives.pitch_lift_coefficient * (dimensions.main_plane_chord / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(1)) + 
				...
				output_.aero_force_.drag = dyn_pressure_ * dimensions.main_plane_area * (
					...
					(derivatives.pitch_drag_coefficient * (dimensions.main_plane_chord / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(1)) +
				...
				output_.aero_force_.side_force = dyn_pressure_ * dimensions.main_plane_area * (
					...
					(derivatives.rollrate_sideforce_coefficient * (dimensions.main_plane_span / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(0)) +
					(derivatives.yawrate_sideforce_coefficient * (dimensions.main_plane_span / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(2)) +
				...
				output_.aero_force_.pitch_mom = dyn_pressure_ * dimensions.main_plane_area * dimensions.main_plane_chord * (
					...
					(derivatives.pitchrate_pitch_coefficient * (dimensions.main_plane_chord / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(1)) +
				...
				output_.aero_force_.roll_mom = dyn_pressure_ * dimensions.main_plane_area * dimensions.main_plane_span * (
					...
					(derivatives.rollrate_roll_coefficient * (dimensions.main_plane_span / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(0)) +
					(derivatives.yawrate_roll_coefficient * (dimensions.main_plane_span / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(2)) +
				...
				output_.aero_force_.yaw_mom = dyn_pressure_ * dimensions.main_plane_area * dimensions.main_plane_span * (
					...
					(derivatives.rollrate_yaw_coefficient * (dimensions.main_plane_span / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(0)) +
					(derivatives.yawrate_yaw_coefficient * (dimensions.main_plane_span / (2 * setAirspeed(kinematics))) * kinematics.twist.angular(2)) +
