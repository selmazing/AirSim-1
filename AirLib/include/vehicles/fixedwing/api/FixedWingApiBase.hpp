...
#ifndef air_FixedWingControlServer_hpp
#define air_FixedWingControlServer_hpp

#include "common/Common.hpp"
#include "FixedWingCommon.hpp"
...
protected: //must be implemented
    ...
    virtual LandedState getLandedState() const = 0; // Don't know if this makes sense for a fixed wing, need to think about undercarriage at some point?
    virtual GeoPoint getGpsLocation() const = 0;
    virtual const FixedWingApiParams& GetFixedWingApiParams() const = 0;
...
public: //these APIs uses above low level APIs
    ...
    FixedWingState getFixedWingState() const
    {
        FixedWingState state;
...
protected: //types
    class SingleCall {
    public:
        SingleCall(FixedWingApiBase* api)
            : api_(api) {
    ...
    protected:
        FixedWingApiBase * getVehicleApi()
        {
            return api_;
        }
    ...
    private:
        FixedWingApiBase* api_;
    };

    class SingleTaskCall : public SingleCall
    {
    public:
        SingleTaskCall(FixedWingApiBase* api)
            : SingleCall(api)
        {
            if (isRootCall())
                api->beforeTask();
    ...
    struct StatusLock {
        ...
        StatusLock(const FixedWingApiBase* api)
            : lock_(
