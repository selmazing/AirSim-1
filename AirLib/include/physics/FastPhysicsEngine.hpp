...
private:
    ...
    static void getNextKinematicsNoCollisionEuler(TTimeDelta dt, PhysicsBody& body, const Kinematics::State& current, Kinematics::State& next, Wrench& next_wrench)
    {
        ...
        Vector3r next_orientation = eulerOrientation + (orientation_dot * dt);

    	
        next.pose.orientation = VectorMath::eulerToQuaternion(next_orientation);
        next.pose.position = current.pose.position + (position_dot * dt);
        next.twist.linear = current.twist.linear + (next.accelerations.linear * dt);
        next.twist.angular = current.twist.angular + (next.accelerations.angular * dt);
...
private:
    ...
    static constexpr float kRestingVelocityMax = 0.000000000001f;
