...
void FixedWingApiBase::resetImplementation()
{
    cancelLastTask();
    SingleTaskCall lock(this); //cancel previous tasks
}

bool FixedWingApiBase::takeoff(float timeout_sec)
{
...
bool FixedWingApiBase::land(float timeout_sec)
{
    SingleTaskCall lock(this);
...
bool FixedWingApiBase::goHome(float timeout_sec)
{
    SingleTaskCall lock(this);
...
Waiter FixedWingApiBase::waitForFunction(WaitFunction function, float timeout_sec)
{
    Waiter waiter(getCommandPeriod(), timeout_sec, getCancelToken());
    if (timeout_sec <= 0)
        return waiter;
...
bool FixedWingApiBase::waitForZ(float timeout_sec, float z, float margin)
{
    float cur_z = 100000;
    return waitForFunction([&]() {
        cur_z = getPosition().z();
...
void FixedWingApiBase::setSafetyEval(const shared_ptr<SafetyEval> safety_eval_ptr)
{
    SingleCall lock(this);
    safety_eval_ptr_ = safety_eval_ptr;
}

RCData FixedWingApiBase::estimateRCTrims(float trimduration, float minCountForTrim, float maxTrim)
{
...
float FixedWingApiBase::getObsAvoidanceVelocity(float risk_dist, float max_obs_avoidance_vel) const
{
    unused(risk_dist);
